...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		sprintf(TX_buffer, "%d", ADC_value);	//Return a formatted string
		HAL_UART_Transmit(&huart6, (uint8_t*)TX_buffer, 5, 100);
		HAL_Delay(100);
  }
  /* USER CODE END 3 */
}
...
void UART_Transmit_HH(char *str){
	//HAL_UART_Transmit(&huart6, (uint8_t*)str, strlen(str), 100);
	HAL_UART_Transmit(&huart6, (uint8_t*)str, 5, 100);
}
/* USER CODE END 4 */
